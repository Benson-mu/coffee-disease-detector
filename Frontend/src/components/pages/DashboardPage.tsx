...
/* ========================================================================
   5. NEW VIEW: DASHBOARD HOME (Unchanged)
======================================================================== */
    ...
    <>
        <div className="grid md:grid-cols-2 bg-gray-900/90 rounded-2xl shadow-2xl border-l-4 border-amber-500 overflow-hidden mb-8">
            <div className="p-8">
               ...
               bg-amber-500 hover:bg-amber-600 text-gray-900 shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
...
/* ========================================================================
   6. NEW VIEW: SCAN PAGE (Updated logic)
======================================================================== */
...
const ScanPage: React.FC<ScanPageProps> = ({
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files?.length) {
            ...
            // Cleanup previous preview object URL to prevent memory leaks
            if (previewUrl) URL.revokeObjectURL(previewUrl);

            setSelectedFile(file);
            setPreviewUrl(URL.createObjectURL(file));
            setUploadMessage(null);
            // Clear result when selecting a new image to avoid confusion
            setResults([]);
        }
    };
    ...
    const handleUpload = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Set result as the ONLY result (clears previous history)
            setResults([newResult]);
            
            if (data.save_status === 'SAVED_SUCCESS') {
                setUploadMessage({ text: 'Scan complete and saved.', type: 'success' });
            }

            // DO NOT clear selectedFile or previewUrl here so it stays visible in the box
    ...
    }, [selectedFile, userToken, userEmail, previewUrl, setResults]);
            ...
            <div className="lg:col-span-1 space-y-8">
                <div className="bg-gray-800/90 p-6 rounded-2xl shadow-xl h-fit sticky top-[90px] border border-gray-700">
                    ...
                    <form onSubmit={handleUpload}>
                        <div className="mb-4">
                            <label className="block text-sm font-medium mb-2">Upload Coffee Leaf Image (JPG/PNG)</label>
                        ...
                        <button
                            ...
                            className={`w-full py-3 rounded-xl font-semibold flex items-center justify-center space-x-2 transition-all ${isLoading || !selectedFile
            ...
            <div className="lg:col-span-2 bg-gray-800/90 p-6 rounded-2xl shadow-xl border border-gray-700">
                ...
                {results.length === 0 ? (
                    <div className="text-center py-12 bg-gray-900/80 rounded-xl text-amber-400 border border-gray-700">
                        ...
                        <p>Waiting for scan... Upload an image to start.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {results.map((result, idx) => (
                            <div key={idx} className="p-4 border border-amber-700 rounded-xl bg-gray-900/80 shadow-lg animate-in fade-in duration-500">
                                    ...
                                    <div className="flex flex-col items-end space-y-2">
                                        ...
                                        {result.scan_id && (
                                            <button
                                                ...
                                                className="bg-red-700 hover:bg-red-600 text-white p-2 rounded-full shadow-md"
                                            >
...
const DashboardPage: React.FC<DashboardPageProps> = ({ userToken, userId, userEmail, onLogout }) => {
    ...
    const handleDeleteScan = useCallback(async (scanId: string | number | undefined) => {
        ...
        try {
            ...
            setResults([]); // Clear the current result display on delete
        } catch (e) {
